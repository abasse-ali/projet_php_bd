...
// Ce fichier est destiné à contenir des fonctions PHP réutilisables
// dans tout le projet.

/**
 * Retourne un icône SVG inline pour remplacer les émojis dans l'interface.
 * Toutes les icônes utilisent currentColor afin d'hériter de la couleur du texte parent.
 *
 * @param string $name Le nom de l'icône à retourner.
 * @param string $extra_class Classes CSS additionnelles (optionnel).
 * @return string Le code SVG inline.
 */
...
/**
 * Mappe un libellé météo en français vers le nom de son icône.
 * Utilisé à l'affichage du journal météo pour récupérer l'icône
 * correspondant à la description stockée en base.
 */
 ...
 * L'Administrateur (Super-Admin) a systématiquement accès à tout.
 */
function a_role($roles) {
    if (!isset($_SESSION['roleU'])) { // Utilisation de roleU
        return false;
    }
    
    // Le Super-Admin a tous les droits (Contournement des restrictions)
    if ($_SESSION['roleU'] === 'Administrateur') { // Utilisation de roleU
        return true;
    }
    ...
    return in_array($_SESSION['roleU'], $roles); // Utilisation de roleU
}

/**
 * Bloque l'accès si l'utilisateur n'a pas le bon rôle, en le redirigeant vers l'accueil.
 */
 ...
 * @param PDO $bdd L'objet PDO pour la connexion à la base de données.
 * @return string Le nom complet de l'utilisateur ou 'Utilisateur' par défaut.
 */
function obtenir_nom_complet(PDO $bdd) {
    if (!isset($_SESSION['id_utilisateur'])) {
        return 'Utilisateur';
    }
    $requeteNom = $bdd->prepare("SELECT prenomU, nomU FROM Utilisateur WHERE id_utilisateur = ?");
    $requeteNom->execute([$_SESSION['id_utilisateur']]);
    $uData = $requeteNom->fetch();
    if ($uData) {
        return htmlspecialchars($uData['prenomu'] . ' ' . $uData['nomu']);
    }
    return htmlspecialchars($_SESSION['nom'] ?? 'Utilisateur'); // Fallback to session name
}

/**
 * Récupère l'attribution active d'une parcelle pour un utilisateur.
 * @param PDO $bdd L'objet PDO pour la connexion.
 * @param int $id_utilisateur L'identifiant de l'utilisateur.
 * @return array|false Les données de l'attribution et de la parcelle, ou false.
 */
function obtenir_attribution_active(PDO $bdd, $id_utilisateur) {
    // Mise à jour des noms de colonnes pour correspondre au nouveau schéma de BDD.
    $requete = $bdd->prepare("
...
/**
 * Récupère l'ensemble des cultures liées à une attribution donnée.
 * @param PDO $bdd L'objet PDO.
 * @param int $id_attribution L'identifiant de l'attribution.
 * @return array Le tableau des cultures avec les noms des variétés.
 */
...
/**
 * Propage une alerte sanitaire vers les exploitants des parcelles VOISINES
 * via l'association réflexive est_voisine_de. Insère une Notification (typeN='Alerte')
 * pour chaque utilisateur concerné (sauf l'auteur du signalement) ainsi que pour les Tuteurs.
 *
 * @param PDO    $bdd            Connexion BDD.
 * @param int    $id_culture     La culture sur laquelle l'alerte a été déclarée.
 * @param string $nom_menace     Nom de la menace (ex: 'Mildiou').
 * @param string $gravite        Niveau de gravité.
 * @param int    $id_auteur      Utilisateur qui déclare l'alerte (exclu des notifs).
...
function propager_alerte_voisinage(PDO $bdd, $id_culture, $nom_menace, $gravite, $id_auteur) {
    // 1. Identifier la parcelle d'origine + l'exploitant et le secteur/numéro
    $requete = $bdd->prepare("
    ...
    // 2. Trouver les exploitants ACTIFS des parcelles voisines (association réflexive)
    //    + les Tuteurs (qui doivent toujours être au courant).
    //    On exclut l'auteur du signalement et on déduplique par utilisateur.
    $requete = $bdd->prepare("
    ...
    // 3. Insérer une notification ciblée pour chacun
    ...
    foreach ($destinataires as $id_dest) {
        $ins->execute([$titre, $contenu, $id_dest]);
        $n++;
    }
    return $n;
}
...
/**
 * Récupère les notifications non lues d'un utilisateur, triées par date.
 * @param PDO $bdd L'objet PDO.
 * @param int $id_utilisateur L'identifiant de l'utilisateur.
 * @param int $limit Le nombre maximum de notifications à récupérer (défaut 5).
 * @return array Le tableau des notifications.
 */
function obtenir_notifications_non_lues(PDO $bdd, $id_utilisateur, $limit = 5) {
    // Mise à jour de la colonne 'lu' en 'est_lue' pour correspondre au nouveau schéma.
    $requete = $bdd->prepare("SELECT * FROM Notification WHERE id_utilisateur_recoit = ? AND est_lue = false ORDER BY date_envoi DESC LIMIT ?");
    // L'utilisation de bindValue empêche les erreurs de typage lors de l'émulation LIMIT par PDO
