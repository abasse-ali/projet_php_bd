...
// Les emojis sont retirés pour un affichage plus professionnel
if (isset($_GET['msg'])) {
...
// Récupération sécurisée des cultures en cours
$cultures = [];
try {
...
// Regroupement intelligent des parcelles par secteur
$secteurs = [];
$donnees_js = []; // Préparation des données pour le menu Javascript
foreach ($parcelles as $p) {
    $secteurs[strtoupper($p['secteurp'])][] = $p;
    
    // On relie les cultures à cette parcelle
    $cultures_parcelle = [];
    foreach($cultures as $c) { if ($c['id_attr'] == $p['id_attribution']) $cultures_parcelle[] = $c['nom_variete']; }
...
// 2. Récupération des candidats éligibles (Visiteurs, ou Adhérents SANS parcelle)
$visiteurs_attente = [];
try {
    // La requête est maintenant fixe, basée sur le schéma SQL officiel.
...
<!-- Utilisation de la balise sémantique main au lieu d'une simple div -->
<main class="container">
    ...
    <div class="dashboard-grid">
        <!-- COLONNE GAUCHE : Le Plan interactif -->
        <section class="main-column">
            <article class="card">
                ...
                <div class="secteurs-grid">
                    <!-- Boucle d'affichage des secteurs -->
                    <?php foreach ($secteurs as $nom_secteur => $parcelles_secteur): ?>
                        <section class="secteur-zone">
                            ...
                            <div class="plan-jardin">
                                <!-- Boucle d'affichage des parcelles (DRY) -->
        ...
        <aside class="side-column">
            <article class="form-card sticky-top">
                ...
                <!-- Panneau pour une parcelle LIBRE -->
                ...
                <!-- Panneau pour une parcelle OCCUPÉE -->
...
<!-- Script JavaScript natif pour gérer l'interactivité de la carte -->
<script>
    const dataParcelles = <?= json_encode($donnees_js) ?>;

    function selectParcelle(id) {
        // 1. Réinitialiser la sélection visuelle
        ...
        // 3. Remplir et afficher le bon panneau
