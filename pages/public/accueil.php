<?php
/**
 * pages/public/accueil.php
 *
 * Page d'accueil principale de la plateforme La Bòstia Verda.
 * Fait office de vitrine publique et de tableau de bord global.
 * Ce script calcule dynamiquement les statistiques d'occupation du jardin,
 * des récoltes cumulées sur l'année et de la disponibilité du matériel,
 * puis oriente l'utilisateur (visiteur ou connecté) via un parcours sémantique.
 */
...
/**
 * @var int $nb_parcelles Nombre de parcelles actuellement attribuées et actives.
 */
...
/**
 * @var int $nb_adherents Nombre total de membres possédant le rôle officiel d'Adhérent.
 */
$requete_adherents = $bdd->query("SELECT COUNT(*) FROM Utilisateur WHERE roleU = 'Adhérent'");
$nb_adherents = $requete_adherents->fetchColumn();

/**
 * @var float|int $kg_recoltes Somme cumulée des récoltes de l'année en cours (2026).
 * Utilisation de COALESCE pour intercepter les retours NULL et renvoyer 0 de manière sécurisée.
 */
...
try {
    ...
    // Dispositif anti-crash : protège l'affichage public si la table ou la colonne est temporairement altérée
    $kg_recoltes = 0;
}

/**
 * @var int $nb_outils Nombre d'outils opérationnels et disponibles au cabanon.
 */
...
<?php
// Inclusion du pied de page (Architecture 3 Tiers - Vue)
